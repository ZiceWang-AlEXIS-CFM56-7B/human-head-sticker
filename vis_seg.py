...
def img_save_and_viz(
    ...
    image = image.data.numpy() if isinstance(image, torch.Tensor) else image  # 确保image是numpy数组
...
def main():
    ...
    inference_dataloader = torch.utils.data.DataLoader(
        ...
        collate_fn=lambda x: (
            [item[0] for item in x],  # image paths
            [np.ascontiguousarray(item[1]) for item in x],  # orig images
            torch.stack([item[2] for item in x])  # preprocessed images
        )
    )
    ...
    # 修改3/3: 添加显式类型转换和连续内存处理
    # for batch_idx, (batch_image_name, batch_orig_imgs, batch_imgs) in tqdm(
    #     enumerate(inference_dataloader), total=len(inference_dataloader)
    # ):
    for batch_idx, batch in tqdm(enumerate(inference_dataloader), total=len(inference_dataloader)):
        batch_image_name, batch_orig_imgs, batch_imgs = batch
        
        # 确保所有图像都是连续的numpy数组
        batch_orig_imgs = [np.ascontiguousarray(img) for img in batch_orig_imgs]
